...
author: "Group members: add names and student numbers"
date: "`r Sys.Date()`"
output:
  pdf_document:
    toc: false
...
The time-series and event analyses show that the rental penalty does not develop in the same way everywhere. Some countries show persistent or increasing gaps, while others show declining gaps after 2020. These differences may reflect national housing systems, rent regulation, mortgage conditions, housing supply and income support policies. However, the analysis is descriptive and cannot prove which factor caused the changes.
...
Before submission, we need to check that:

- the report is no longer than 10 pages;
- all names, student numbers, tutorial group and lecturer are added;
- all figures are discussed in the text;
- the GitHub repository link is included;
- the README explains how to reproduce the project;
- `renv.lock` and the RMarkdown file are included in the repository;
- conclusions do not make causal claims that the data cannot support.
